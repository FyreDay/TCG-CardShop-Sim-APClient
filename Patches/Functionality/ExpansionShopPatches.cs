...
public class ExpansionShopPatches
{
    ...
    public class Init
    {
        ...
        static void Postfix(ExpansionShopPanelUI __instance, ExpansionShopUIScreen expansionShopUIScreen, int index, bool isShopB)
        {
            ...
            if (Plugin.ArchipelagoHandler.slotData.AutoRenovate)
            {
                ...
                if(index < Plugin.ArchipelagoHandler.GetItemCount(isShopB ? GenericItemMapping.SHOP_EXPANSION_B : GenericItemMapping.SHOP_EXPANSION_A))
                {
            ...
            if (isShopB)
            {
                hasAPItem = Plugin.ArchipelagoHandler.GetItemCount(GenericItemMapping.SHOP_EXPANSION_B) > index;
            }
            else
            {
                hasAPItem = Plugin.ArchipelagoHandler.GetItemCount(GenericItemMapping.SHOP_EXPANSION_A) > index;
            }
    ...
    public class OnClick
    {
        static bool Prefix(ExpansionShopPanelUI __instance)
        {
            ...
            {
                hasAPItem = Plugin.ArchipelagoHandler.GetItemCount(GenericItemMapping.SHOP_EXPANSION_B) > __instance.m_Index;
            }
            else
            {
                hasAPItem = Plugin.ArchipelagoHandler.GetItemCount(GenericItemMapping.SHOP_EXPANSION_A) > __instance.m_Index;
            }
            bool atLevel = true; // (CPlayerData.m_ShopLevel + 1) >= m_LevelRequired;
